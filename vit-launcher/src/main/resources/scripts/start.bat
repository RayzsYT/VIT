...
if not exist "%JAVA_SANITY_CHECK%" (
    ...
    echo So, let's gets started. First, we gotta install Java. This will take a while...
    title Preparing Java...

    if not exist "%JAVA_DIR%" mkdir "%JAVA_DIR%"

    echo Let's start downloading! :D
...
if not exist "%VIT_JAR_FILE%" (
    ...
    echo Let's download the latest version of VIT...
...
:: Check if VIT has an updated version available for use.
cd /d %VIT_DIR%
